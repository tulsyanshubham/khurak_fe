...
export default function Footer() {
    ...
    return (
        <div className='flex flex-col items-center justify-center w-full gap-3 py-2 border-t-2 mt-2 bg-green-50 dark:bg-footergDarkBg dark:text-white'>
            <div className='flex items-center justify-evenly w-full flex-wrap py-7 gap-10 dark:text-gray-100'>
                ...
                <div className='flex flex-col gap-2'>
                    ...
                    <a href={`tel:${data.phone}`} className='flex gap-1 text-lg justify-center md:justify-start' >
                        <Image width={500} height={500} src={call} alt='call' className="invert-[.75] w-7 dark:invert-[.25]" /><span>{data.phone}</span>
                    </a>
                    <a href={`mailto:${data.email}`} className='flex gap-1 text-lg justify-center md:justify-start' >
                        <Image width={500} height={500} src={mail} alt='mail' className="invert-[.75] w-7 dark:invert-[.25]" /><span>{data.email}</span>
                    </a>
                </div>
                <div>
            ...
            <div>Copyright &copy; {year} [<span className='text-green-700 dark:text-green-500 font-bold'>KHURAK</span>]. All rights reserved.</div>
        </div>
    )
}
