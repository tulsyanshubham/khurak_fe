export const influencerReviews = [
    {
        heading: "Game-Changer for Healthy Eating!",
        content: "Khuraak has completely transformed my approach to healthy eating. The food is not only delicious but also guilt-free. I no longer have to choose between taste and health. The meals are perfectly balanced, nutritious, and truly satisfying. I can confidently say that Khuraak has made it so much easier to maintain a healthy lifestyle without sacrificing flavor. Highly recommend it to anyone looking for healthy and convenient meal options!",
        image: "/images/influencers/1.png",
        name: "Ranveer Allahbadia",
        url: "https://www.instagram.com/ranveerallahbadia/"
    },
    {
        heading: "Nourishing and Flavorful!",
        content: "Khuraak is a lifesaver! I’ve been ordering from them for a while now, and I couldn’t be happier with the meals. They are always so fresh, and every dish I’ve tried has been packed with flavor. The best part is that the meals are balanced and nourishing, keeping me energized throughout the day. I’m always excited to see what’s on the menu next. Khuraak makes healthy eating so easy and enjoyable!",
        image: "/images/influencers/2.png",
        name: "Sunit Jadhav",
        url: "https://www.instagram.com/sunitjadhavofficial/"
    },
    {
        heading: "Delicious & Nutritious!",
        content: "I’ve been trying Khuraak for a few weeks now, and I’m absolutely hooked! Their meals are packed with flavor and nutrition, making it so easy to stick to a healthy lifestyle. Whether it’s lunch or dinner, the food is always fresh, tasty, and full of the nutrients I need to feel my best. I love that I can enjoy a delicious meal without worrying about unhealthy ingredients. Khuraak has truly changed the way I think about eating well!",
        image: "/images/influencers/3.png",
        name: "Ritik Choudhary",
        url: "https://www.instagram.com/ritik_choudhary__0001/"
    },
    {
        heading: "Perfect for Busy Days!",
        content: "I absolutely love Khuraak’s quick and healthy meals! They’re the perfect solution for my hectic and busy lifestyle. I don’t have to spend hours in the kitchen or compromise on health. With Khuraak, I can enjoy a satisfying, nutritious meal that is both quick and easy to prepare. The flavors are always on point, and I feel so good knowing I’m nourishing my body with healthy ingredients. It’s definitely my go-to food service for busy days!",
        image: "/images/influencers/4.png",
        name: "AADI NAGAR",
        url: "https://www.instagram.com/aadi.nagar/"
    },
];
